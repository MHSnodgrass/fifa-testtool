...
function Sidebar({ isOpen, toggleSideBar }: SideBarProps) {
  ...
  return (
    <aside
      className={`fixed left-0 top-0 h-full bg-surface-container-lowest flex flex-col pt-16 z-40 ${isOpen ? 'w-72' : 'w-20'} transition-all duration-300`}
    >
      ...
      <button
        className={`material-symbols-outlined absolute right-0 top-20 translate-x-1/2 w-8 h-8 rounded-full bg-surface-container-high 
        border border-outline-variant/30 text-secondary hover:text-primary hover:bg-surface-container-highest flex items-center 
        justify-center transition-colors z-50 hover:cursor-pointer ${!isOpen ? 'rotate-180' : ''}`}
      ...
      <nav className="flex flex-col space-y-1">
        ...
        >
          ...
          <span className={transitionClasses}>Events</span>
        </NavLink>
        <NavLink
          to="/teams"
          className={({ isActive }) => (isActive ? activeClasses : inactiveClasses)}
        >
          ...
          <span className={transitionClasses}>Teams</span>
        </NavLink>
        <NavLink
          to="/edit-tournament"
          className={({ isActive }) => (isActive ? activeClasses : inactiveClasses)}
        >
          ...
          <span className={transitionClasses}>Edit Tournament</span>
        </NavLink>
      </nav>
    </aside>
  );
